...
class HandleFiles:
    ...
    @staticmethod
    def check_for_xml_files(folder_input: str) -> list[str]:
        work_path = Path(folder_input)
        ...
        if len(xml_files) == 0:
            xml_files = list(file.name for file in work_path.glob('*.xsl'))

            for index, file in enumerate(xml_files):
                file_path = work_path / file
                new_file = Path(file_path).with_name(file_path.stem)
                file_path.rename(new_file)

                xml_files[index] = new_file.name
    ...
    def create_message_files(
        self, messages: list[Dict[str, Any]], year_folder: bool = False
    ) -> None:
        # Create the output folder with the current year and month
        if year_folder:
            year_month_folder = datetime.now().strftime('%Y%m')
