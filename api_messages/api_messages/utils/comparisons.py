...
class Comparisons:
    def compare(
        operator_str: str, first_value: Any, second_value: Any, extra=None
    ) -> bool:
        """
        ...
        if operator_str in {'BETWEEN', 'NOT BETWEEN'}:
            if extra is None:
                raise ValueError(
                    f'The operator "{operator_str}" requires a third argument '
                    f'(upper bound).'
                )
    ...
    def is_valid_path(*args) -> bool:
        """
        Function that checks if the given value is a valid path.

        Parameters:
        - args: the value or values to be checked.

        Returns:
        - bool: True if the value or values is a valid path, otherwise False.
        """
        if len(args) == 0:
            return False
        elif len(args) == 1:
            if isinstance(args[0], (str, Path)):
                return bool(str(args[0]).strip())
        elif len(args) > 1:
            return_value = False

            for value in args:
                if isinstance(value, (str, Path)):
                    return_value = bool(str(value).strip())
